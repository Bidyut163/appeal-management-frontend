...
export const baseFormSchema = z.object({
    ...
    groundsOfAppeal: z.string().trim().min(1, 'Grounds of appeal is required'),
    ...
    appealDocument: appealDocumentSchema.optional(),
});

export const formSchema = baseFormSchema
    // .extend({
    //     appealDocument: appealDocumentSchema,
    // })
...
export const resubmitAppealSchema = baseFormSchema
    .superRefine((data, ctx) => {
        if (!data.isFiledWithinLimitation && !data.delayReason?.trim()) {
            ctx.addIssue({
                code: 'custom',
...
export type FormInput = z.infer<typeof baseFormSchema>;
// export type CreateAppealFormInput = z.infer<typeof formSchema>;
export type ResubmitFormInput = z.infer<typeof resubmitAppealSchema>;
